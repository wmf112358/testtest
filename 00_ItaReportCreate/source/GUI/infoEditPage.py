...
class InfoEditPage(QWidget):
    def __init__(self, on_back, path="基本情報設定", pattern_type=None):
        ...
        self.section_thread_running = False
    ...
    def show_section_select(self):
        if getattr(self, 'section_thread_running', False):
            # 线程未结束禁止启动新线程
            return
        ...
        # 启动新线程
        self.section_thread_running = True
        thread = SectionExtractThread(file_path, pgmid)
        thread.finished.connect(self.on_section_extracted)
        thread.start()
        self.threads.append(thread)
