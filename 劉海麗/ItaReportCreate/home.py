...
from PyQt5.QtWidgets import (
    ...
    QDialog, QCheckBox, QHeaderView, QStyledItemDelegate, QTextEdit
)
...
class CaseDetailEditPage(QWidget):
    def __init__(self, on_back, case_data, detail_data, section_list):
        ...
        detail_bar_layout.setSpacing(10)
        ...
        #detail_case_layout.setSpacing(4)  # 紧凑且固定间距
...
class MultiHeaderView(QHeaderView):
    def __init__(self, parent=None):
        ...
        self.setFixedHeight(44 + 28)  # 两行表头高度

    def paintSection(self, painter, rect, logicalIndex):
        painter.save()
        border_color = QColor("#BFBFBF")
        bg_color = QColor("#d6e7f4")
        # 强制背景色，不管hover
        if logicalIndex == 0:
            ...
            painter.drawRect(rect.x(), rect.y(), width, 44)
            painter.drawLine(rect.x(), rect.y() + 44 + 28 - 1, rect.x() + width, rect.y() + 44 + 28 - 1)
            font = painter.font()
            font.setBold(True)
